...
def get_distance(list1: list, list2: list) -> float:
    """we need to calculate euclidean distance between two lists"""
...
def get_standard_deviation(data: list, col_num: int) -> float:
    """calculate standard deviation of column number passed"""
...
def get_standardised_matrix(data: list) -> list:
    ...
    # converting standardize data in standardised matrix
    standardised_matrix = [
...
def get_k_nearest_labels(
    ...
    # sorting distances ascending order
...
def get_mode(k_nearest_labels: list) -> int:
    ...
    # return statistics.mode using k_nearest_labels
...
def classify(standardised_data: list,
    ...
    """
    This function use 'get_k_nearest_labels'  and 'get_mode' methods
...
def get_accuracy(correct_data_labels: list, data_labels: list) -> float:
    ...
    matrixes have exactly the same valuesthen the accuracy is of 100%.
    If only half of the values of both tables matchthen the accuracy is of 50%

    """
    count = 0
    N = len(correct_data_labels)
    # converting list of lists data to list.
...
def run_test():
    """
    This function create one matrix for : correct_data_labels,
    learning_data and learning_data_labels (using load_from_csv). firstly,it
    standardise the matrix data and the matrix learning_data (using
    get_standardised_matrix). Secondly, it run the algorithm  and
    calculate the accuracy  for a series of experiments.
    ...
    if with k = 4 the accuracy is 72.5% it should show:
    k=4, Accuracy = 72.5%
    """
    data = load_from_csv("C:/Users/KUNAL PRADHAAN/Downloads/Data.csv")
    correct_data_labels = load_from_csv("C:/Users/KUNAL PRADHAAN/Downloads/Correct_Data_Labels.csv")
    learning_data = load_from_csv("C:/Users/KUNAL PRADHAAN/Downloads/Learning_Data.csv")
    learning_data_labels = load_from_csv("C:/Users/KUNAL PRADHAAN/Downloads/Learning_Data_Labels.csv")
    

    # geting standardised matrix for data matrix
    standardised_data = get_standardised_matrix(data)
    # geting standardised matrix for learning data matrix
    standardised_learning_data = get_standardised_matrix(learning_data)
    # calculating accuracy for different values of k.
    for k in range(3, 16):
